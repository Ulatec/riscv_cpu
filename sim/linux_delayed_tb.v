...
// ============================================================================
// Linux Boot with Delayed Memory — tests pipeline stall/unstall behavior
// Same as linux_shell_tb but with MEM_DELAY cycles per memory access.
// If the doubled-character bug reproduces here, it's a CPU logic bug
// triggered by multi-cycle memory latency.
// ============================================================================
module linux_delayed_tb;
    ...
    parameter MEM_DELAY = 2;  // 2-cycle delay per memory access
    ...
    initial begin
        ...
        $display("=== Linux Boot with Delayed Memory (MEM_DELAY=%0d) ===", MEM_DELAY);
        $display("RAM: %0d MB  Max cycles: %0dM", RAM_SIZE/(1024*1024), MAX_CYCLES/1000000);
        $display("Each memory access takes %0d cycles (simulating AXI DDR latency)", MEM_DELAY);
        ...
        if (found_rela) begin
            ...
            $display("WARNING: No relocation table found");
        end
        ...
        for (i = 0; i < MAX_CYCLES; i = i + 1) begin
            ...
            if (!saw_kernel_virt && pc >= 32'hC0000000) begin
                saw_kernel_virt = 1;
                $display("[%0d] KERNEL VIRTUAL pc=0x%08x", i, pc);
                $fflush();
            end
            ...
            if (trap_taken && (trap_cause == 12 || trap_cause == 13 || trap_cause == 15)) begin
                ...
                if (page_faults <= 3)
                    $display("[%0d] PGF #%0d cause=%0d pc=0x%08x",
        ...
        $display("\n\n=== RESULTS (MEM_DELAY=%0d) ===", MEM_DELAY);
